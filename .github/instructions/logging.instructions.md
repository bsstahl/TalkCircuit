---
description: "Logging requirements and conventions for TalkCircuit implementation work"
applyTo: '**/*.cs'
---

## Logging requirements for TalkCircuit
...
The following rule is mandatory for all TalkCircuit implementation work:
...
* loading or scanning a CFP or submission catalog
* processing a batch of conferences, proposals, or submissions
* querying or resolving events or outcomes with a meaningful result count
...
* `Loading TalkCircuit CFP catalog from {DataRoot}`
* `Loaded {SubmissionCount} submission(s) and {CfpCount} CFP(s)`
* `Submitting proposal {ProposalId} to conference {ConferenceId}`
* `Completed CFP sync in {ElapsedMs} ms`
...
* `Trace: Loaded submission {SubmissionId} for talk {TalkId} from {SourcePath}`
* `Trace: CFP criteria track={Track}, tags={Tags}, deadline={Deadline}`
* `Trace: Submission payload for conference {ConferenceId} with {ItemCount} record(s)`
...
* `Skipping malformed CFP file {SourcePath} because the JSON could not be parsed`
* `No submission outcome was found for talk {TalkId}; continuing with empty result`
* `Using default deadline value because the source value was empty`
...
* `Failed to load TalkCircuit catalog from {DataRoot}`
* `Unable to parse submission record {SourcePath}; validation failed for required field {FieldName}`
* `Submission workflow failed for conference {ConferenceId}; upstream response was rejected`
...
* before returning a result set from a query or CFP operation
...
```csharp
logger.LogInformation("Loading TalkCircuit catalog from {DataRoot}", dataRoot);
logger.LogTrace("Loaded submission {SubmissionId} for talk {TalkId}", submission.Id, submission.TalkId);
logger.LogWarning("Skipping malformed CFP file {SourcePath}", sourcePath);
```

Avoid:

```csharp
logger.LogInformation("Loading TalkCircuit catalog from " + dataRoot);
logger.LogTrace("Loaded submission " + submission.Id + " for talk " + submission.TalkId);
```
...
For TalkCircuit, logging should be aligned to the domain and the system boundaries described in the repo.
